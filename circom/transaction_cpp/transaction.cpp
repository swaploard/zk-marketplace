...
void Transaction_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void Transaction_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 261;}
...
ctx->componentMemory[coffset].templateName = "Transaction";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
uint csoffset = mySignalStart+3;
for (uint i = 0; i < 1; i++) {
std::string new_cmp_name = "isNegative";
...
cmp_index_ref_load = 0;
{{
Fr_eq(&expaux[0],&ctx->signalValues[ctx->componentMemory[mySubcomponents[0]].signalStart + 0],&circuitConstants[1]); // line circom 18
}}
...
Transaction_2_create(1,0,ctx,"main",0);
Transaction_2_run(0,ctx);
}
