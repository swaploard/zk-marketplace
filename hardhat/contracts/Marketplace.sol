...
contract NFTMarketplace is ReentrancyGuard {
    ...
    function listItem(address tokenAddress, uint256 tokenId, uint256 price) external nonReentrant {
        ...
        emit NFTListed(listingCounter, msg.sender, tokenAddress, tokenId, price);
    }
    ...
    function buyItem(uint256 listingId) external payable nonReentrant {
        ...
        (address royaltyReceiver, uint256 royaltyAmount) = _getRoyaltyInfo(listing.tokenAddress, listing.tokenId, salePrice);
        ...
        // Payout: first to seller then royalty recipient.
    ...
    function createAuction(
        ...
        emit AuctionCreated(auctionCounter, msg.sender, tokenAddress, tokenId, startingPrice, duration);
    }
    ...
    function finalizeAuction(uint256 auctionId) external nonReentrant {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            (address royaltyReceiver, uint256 royaltyAmount) = _getRoyaltyInfo(auction.tokenAddress, auction.tokenId, salePrice);
            uint256 sellerAmount = salePrice - royaltyAmount;
    ...
    function _getRoyaltyInfo(address tokenAddress, uint256 tokenId, uint256 salePrice)
        internal
        returns (address royaltyReceiver, uint256 royaltyAmount)
    {
        // Use try/catch in case the NFT does not implement EIP-2981.
        try IERC2981(tokenAddress).royaltyInfo(tokenId, salePrice) returns (address receiver, uint256 amount) {
